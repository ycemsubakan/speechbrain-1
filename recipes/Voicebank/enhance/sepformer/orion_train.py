...
class SEBrain(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        if self.hparams.use_freq_domain:
           ...
           # try:
           #     clean_spec_len = self.compute_feats(batch.clean_sig.data).shape[1]
           # except:
           # predict_spec = predict_spec[:, :clean_spec_len, :]
    ...
    def fit_batch(self, batch):
        ...
        if self.hparams.use_speed_perturb:
            ...
            # normalize noises
            #en_noise = ((noise**2).sum(-1).sqrt())
            #en_targets = (targets_speed**2).sum(-1).sqrt()

            #print( (en_targets / en_noise).sqrt() )
            #print( (en_targets / en_noise).sqrt().mean())


            #noise = noise / en_noise            
            #targets_speed = targets_speed / en_targets
    ...
    def on_stage_end(self, stage, stage_loss, epoch=None):
        ...
        if stage == sb.Stage.VALID:
            ...
            # report_objective({'name':'objective', 'type': 'objective', 'value':stats})
            orion_stats = {
                "loss": stage_loss,
                "pesq": -self.pesq_metric.summarize("average"),
                "stoi": -self.stoi_metric.summarize("average"),
            ...
            # hp.report_result(orion_stats)

            if (self.hparams.epoch_counter.current > 1) and (stats['pesq'] < 1.80):
                report_objective(-stats['pesq'])
                quit()

            if (self.hparams.epoch_counter.current > 5) and (stats['pesq'] < 2.00):
                report_objective(-stats['pesq'])
                quit()
            
            if (self.hparams.epoch_counter.current > 25) and (stats['pesq'] < 2.2):
                report_objective(-stats['pesq'])
                quit()
...
if __name__ == "__main__":
    ...
    with hp.hyperparameter_optimization(objective_key="pesq") as hp_ctx: # <-- Initialize the context
